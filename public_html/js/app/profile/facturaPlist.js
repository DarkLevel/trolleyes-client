...
moduleFactura.controller('facturaPlistUserController', ['$scope', '$http', '$location', 'toolService', '$routeParams', '$anchorScroll',
    function ($scope, $http, $location, toolService, $routeParams, $anchorScroll) {
        ...
        $scope.resetOrder = function () {
            $location.url('profile/factura/plist/' + $scope.rpp + '/' + $scope.page);
        };

        $scope.crear = function () {
            $location.url('profile/factura/create');
        };
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/trolleyes/json?ob=factura&op=getcountfacturasuser'
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/trolleyes/json?ob=factura&op=getpagefacturasuser&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        }).then(function (response) {
            $scope.status = response.status;
        ...
        $scope.crearPDF = function (id) {
            ...
            $http({
                method: 'GET',
                url: 'http://localhost:8081/trolleyes/json?ob=linea&op=getpagelineauser&rpp=10000&page=1&id_fact=' + id
            }).then(function (response) {
